...
- [x] 完成（`_fillMSymbolsBufferChar` 輔助方法；`cin_has_charStrLow` 預計算；`in_normal_input_mode` 預計算；`.index()` 改 `enumerate`；msymbols/ctrlsymbols 4 重複塊消除；`_handleMSymbolsInMultifunctionMode` + `_handleCtrlSymbols` 抽方法）
