...
- 長期使用效能修正：
  ...
  - 排序時不再每次完整正規化所有上下文資料。
- 改為純上下文預測（2026-06-12，使用者回饋定案）：**智慧選字不再做任何全域頻率重排**——沒有「前一字上下文」訊號時，候選順序永遠等於碼表順序，徹底保護選字鍵的肌肉記憶與空白鍵預設輸出。只有「在相同前一字之後選過這個字」（如常打「詹智丞」，打完「詹」再組「智」的字碼）才會把該字提前；全域次數與新近度僅在同一上下文有多個後續字時當次要排序訊號。
- 排序門檻與上下文清理（2026-06-12）：
  - 修正「誤選一次就永久重排」：count 永不衰減，選過一次的字（≈1 分）會永遠壓過沒選過的字（0 分），把碼表設計好的常用字順序打亂；更糟的是空白鍵直接出字走排序後的 `candidates[0]`，等於**誤選一次就偷換了空白鍵的預設輸出**。現在需要選過兩次以上、或在相同前一字的上下文中選過，才參與重排。
  - 修正殘留上下文：`lastCommitString` 在切換視窗/欄位後不會清除，前一個欄位的最後一字會錯誤加權下一個欄位的排序；現在組字被強制中斷（含焦點離開）時清空。
- 計分改用對數刻度（2026-06-11）：
...
- 視覺現代化：淺色背景 + 白色圓角卡片、強調色選取狀態、核取方塊 accent 色、現代化套用按鈕。
- 底部動作列輕量化（含「變更會在套用後立即生效」提示），不再是整條黑色導覽列。
- 窄視窗時側欄自動轉為頂部分組橫向導覽，移除漢堡選單（原文件記載的「發現設定入口成本偏高」問題就此解決）。
- 結構相容：保留 Bootstrap 分頁機制與所有控制項 id（`#navbars`、`#intelligent_ddmenu`、`#keyboard_ddmenu`、`#ok`），config.js 零修改；分組改為單一清單，跨分組切換的 active 狀態處理正確。

## 排除聯想字詞（2026-06-12）

- 新增「排除聯想字詞」：內建詞庫中不想看到的聯想字詞（例如人名）可在設定頁「聯想字詞」分頁下方的排除表移除，語法與詞庫相同（「毛=澤東」），顯示與「優先以聯想字詞排序」時皆會略過；存於使用者設定目錄 `excludephrase.dat`，套用後即時生效。
- 順手修正：聯想字詞顯示時會把內建詞庫的詞 append 進使用者詞庫的內部清單（執行期資料污染）；過濾器一律回傳新清單，並以單元測試鎖住。
